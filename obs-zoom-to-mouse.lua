...
-- Prefer the newer _info2 API if available (OBS changed the sceneitem transform API names),
-- but fall back to the older functions for older builds.
...
function release_sceneitem()
    ...
    if sceneitem ~= nil then
        ...
        if sceneitem_info_orig ~= nil then
            ...
            if has_sceneitem_setinfo then
                sceneitem_set_info_fn(sceneitem, sceneitem_info_orig)
            else
                log("WARNING: obs_sceneitem_set_info not available — cannot restore transform info on this OBS version.")
            end
...
function refresh_sceneitem(find_newest)
    ...
    if sceneitem ~= nil then
        ...
        if has_sceneitem_getinfo then
            sceneitem_get_info_fn(sceneitem, sceneitem_info_orig)
        else
            -- Fallback: populate a reasonable default transform so script can continue
            sceneitem_info_orig.bounds_type = obs.OBS_BOUNDS_SCALE_INNER
            sceneitem_info_orig.bounds_alignment = 5
            sceneitem_info_orig.bounds.x = 0
            sceneitem_info_orig.bounds.y = 0
            sceneitem_info_orig.scale.x = 1
            sceneitem_info_orig.scale.y = 1
            log("WARNING: obs_sceneitem_get_info not available — using fallback transform info.")
        end
        ...
        if has_sceneitem_getinfo then
            sceneitem_get_info_fn(sceneitem, sceneitem_info)
        else
        ...
        if sceneitem_info.bounds_type == obs.OBS_BOUNDS_NONE then
            ...
            if has_sceneitem_setinfo then
                sceneitem_set_info_fn(sceneitem, sceneitem_info)
            else
                log("WARNING: obs_sceneitem_set_info not available — cannot convert transform on this OBS version.")
            end
